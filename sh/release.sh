...
then
    ...
    zip -r $WHEREIAM/build/$ENV/$BUILD_NUMBER_NEW/my-youtube-$BUILD_NUMBER_NEW-v$MANIFEST_VERSION.zip * -x "*.DS_Store" -x "__MACOSX" -x "*_original.css"
    echo "Extension folder compressed: my-youtube-$BUILD_NUMBER_NEW-v$MANIFEST_VERSION.zip"
